...
namespace Tests.MySqlTests
{
    ...
    public class Select : MySqlProviderTests
    {
        ...
        {
            var statement = Sql.Select.Output(Sql.Name("id")).From("table1").Intersect(Sql.Select.Output(Sql.Name("table1_id")).From("table2"));

            var command = Provider.GetCommand(statement);

            Assert.IsNotNull(command);
            Assert.AreEqual("SELECT `table1`.`id` FROM `table1` WHERE EXISTS( ( SELECT * FROM `table2` WHERE `table1`.`id` = `table2`.`table1_id` ) );", command.CommandText);
        }

        [TestMethod]
        public void SelectIntersectSelectMulti()
        {
            var statement = Sql.Select.Output(Sql.Name("id"), Sql.Name("name")).From("table1").Intersect(Sql.Select.Output(Sql.Name("table1_id"), Sql.Name("name")).From("table2"));

            var command = Provider.GetCommand(statement);

            Assert.IsNotNull(command);
            Assert.AreEqual("SELECT `table1`.`id`, `table1`.`name` FROM `table1` WHERE EXISTS( ( SELECT * FROM `table2` WHERE `table1`.`id` = `table2`.`table1_id` AND `table1`.`name` = `table2`.`name` ) );", command.CommandText);
        }

        [TestMethod]
        public void SelectIntersectSelectWhere1()
        {
            var statement = Sql.Select.Output(Sql.Name("id"), Sql.Name("name")).From("table1").Where(Sql.Name("name").IsEqual(Sql.Scalar("name3"))).Intersect(Sql.Select.Output(Sql.Name("table1_id"), Sql.Name("name")).From("table2"));

            var command = Provider.GetCommand(statement);

            Assert.IsNotNull(command);
            Assert.AreEqual("SELECT `table1`.`id`, `table1`.`name` FROM `table1` WHERE EXISTS( ( SELECT * FROM `table2` WHERE `table1`.`id` = `table2`.`table1_id` AND `table1`.`name` = `table2`.`name` ) ) AND `table1`.`name` = N'name3';", command.CommandText);
        }

        [TestMethod]
        public void SelectIntersectSelectWhere2()
        {
            var statement = Sql.Select.Output(Sql.Name("id"), Sql.Name("name")).From("table1").Intersect(Sql.Select.Output(Sql.Name("table1_id"), Sql.Name("name")).From("table2").Where(Sql.Name("name").IsEqual(Sql.Scalar("name3"))));

            var command = Provider.GetCommand(statement);

            Assert.IsNotNull(command);
            Assert.AreEqual("SELECT `table1`.`id`, `table1`.`name` FROM `table1` WHERE EXISTS( ( SELECT * FROM `table2` WHERE `table1`.`id` = `table2`.`table1_id` AND `table1`.`name` = `table2`.`name` AND `table2`.`name` = N'name3' ) );", command.CommandText);
        }

        [TestMethod]
        public void SelectExceptSelect()
        {
            var statement = Sql.Select.Output(Sql.Name("id")).From("table1").Except(Sql.Select.Output(Sql.Name("table1_id")).From("table2"));

            var command = Provider.GetCommand(statement);

            Assert.IsNotNull(command);
            Assert.AreEqual("SELECT `table1`.`id` FROM `table1` WHERE NOT EXISTS( ( SELECT * FROM `table2` WHERE `table1`.`id` = `table2`.`table1_id` ) );", command.CommandText);
        }

        [TestMethod]
        public void SelectExceptSelectMulti()
        {
            var statement = Sql.Select.Output(Sql.Name("id"), Sql.Name("name")).From("table1").Except(Sql.Select.Output(Sql.Name("table1_id"), Sql.Name("name")).From("table2"));

            var command = Provider.GetCommand(statement);

            Assert.IsNotNull(command);
            Assert.AreEqual("SELECT `table1`.`id`, `table1`.`name` FROM `table1` WHERE NOT EXISTS( ( SELECT * FROM `table2` WHERE `table1`.`id` = `table2`.`table1_id` AND `table1`.`name` = `table2`.`name` ) );", command.CommandText);
        }

        [TestMethod]
        public void SelectExceptSelectWhere1()
        {
            var statement = Sql.Select.Output(Sql.Name("id"), Sql.Name("name")).From("table1").Where(Sql.Name("name").IsEqual(Sql.Scalar("name4"))).Except(Sql.Select.Output(Sql.Name("table1_id"), Sql.Name("name")).From("table2"));

            var command = Provider.GetCommand(statement);

            Assert.IsNotNull(command);
            Assert.AreEqual("SELECT `table1`.`id`, `table1`.`name` FROM `table1` WHERE NOT EXISTS( ( SELECT * FROM `table2` WHERE `table1`.`id` = `table2`.`table1_id` AND `table1`.`name` = `table2`.`name` ) ) AND `table1`.`name` = N'name4';", command.CommandText);
        }

        [TestMethod]
        public void SelectExceptSelectWhere2()
        {
            var statement = Sql.Select.Output(Sql.Name("id"), Sql.Name("name")).From("table1").Except(Sql.Select.Output(Sql.Name("table1_id"), Sql.Name("name")).From("table2").Where(Sql.Name("name").IsEqual(Sql.Scalar("name4"))));

            var command = Provider.GetCommand(statement);

            Assert.IsNotNull(command);
            Assert.AreEqual("SELECT `table1`.`id`, `table1`.`name` FROM `table1` WHERE NOT EXISTS( ( SELECT * FROM `table2` WHERE `table1`.`id` = `table2`.`table1_id` AND `table1`.`name` = `table2`.`name` AND `table2`.`name` = N'name4' ) );", command.CommandText);
        }
        /*[TestMethod]
        public void SelectExceptSelect()
        {
        ...
        public void SelectFullOuterJoin()
        {
            AssertSql(
                ...
                .Output(Sql.Star())
                .From("t1", "o")
                .FullOuterJoin(Sql.Name("t2").As("t"), Sql.Name("t.C1").IsEqual(Sql.Name("o.C1"))),
            "SELECT * FROM `t1` AS `o` LEFT OUTER JOIN `t2` AS `t` ON `t`.`C1` = `o`.`C1` UNION ALL SELECT * FROM `t1` AS `o` RIGHT OUTER JOIN `t2` AS `t` ON `t`.`C1` = `o`.`C1`;");

            AssertSql(Sql.Select
                .Output(Sql.Star())
                .From("t1", "o")
                .FullOuterJoin(Sql.Name("t2"), "t", Sql.Name("t.C1").IsEqual(Sql.Name("o.C1"))),
            "SELECT * FROM `t1` AS `o` LEFT OUTER JOIN `t2` AS `t` ON `t`.`C1` = `o`.`C1` UNION ALL SELECT * FROM `t1` AS `o` RIGHT OUTER JOIN `t2` AS `t` ON `t`.`C1` = `o`.`C1`;");

            AssertSql(Sql.Select
                .Output(Sql.Star())
                .From("t1", "o")
                .FullOuterJoin("t2", "t", Sql.Name("t.C1").IsEqual(Sql.Name("o.C1"))),
            "SELECT * FROM `t1` AS `o` LEFT OUTER JOIN `t2` AS `t` ON `t`.`C1` = `o`.`C1` UNION ALL SELECT * FROM `t1` AS `o` RIGHT OUTER JOIN `t2` AS `t` ON `t`.`C1` = `o`.`C1`;");
        }
